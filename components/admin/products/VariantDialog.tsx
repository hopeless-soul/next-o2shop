...
export interface VariantDialogProps {
  ...
  onSubmit: (dto: CreateVariantDto) => void | Promise<void>
}
...
function fromDto(dto: CreateVariantDto): FormState {
  return {
...
function VariantForm({
  ...
  const [form, setForm] = useState<FormState>(() =>
    initialValues ? fromDto(initialValues) : emptyForm()
  )
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {open && (
          <VariantForm
            key={`${initialValues?.colorName ?? ''}-${initialValues?.size ?? ''}-${initialValues?.colorValue ?? ''}`}
